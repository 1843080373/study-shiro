...
public class LoginAndLogoutController {
	 ...
	 * login请求调用subject.login之后，shiro会将token传递给自定义realm,此时realm会先调用doGetAuthenticationInfo(AuthenticationToken
	 * authcToken )登录验证的方法，验证通过后会接着调用 doGetAuthorizationInfo(PrincipalCollection
	 * principals)获取角色和权限的方法（授权），最后返回视图。
	 * 当其他请求进入shiro时，shiro会调用doGetAuthorizationInfo(PrincipalCollection
	 * principals)去获取授权信息，若是没有权限或角色，会跳转到未授权页面，若有权限或角色，shiro会放行，此时进入真正的请求方法……
	 * 
	 * @param username
	 * @param password
	 * @param model
